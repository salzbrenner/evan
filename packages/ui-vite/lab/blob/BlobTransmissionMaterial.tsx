  ...
  constructor(samples = 6, transmissionSampler = false) {
    ...
    `;
      ...
      shader.vertexShader = shader.vertexShader.replace(
        ...
        vec3 transformed = vec3(position * (noise * pow(vertexDistort, 2.0) + radius));
        `
      );
    };

    Object.keys(this.uniforms).forEach((name) => {
      const _name = name as keyof MeshTransmissionMaterialImpl["uniforms"];
      Object.defineProperty(this, name, {
        get: () => this.uniforms[_name].value,
        set: (v) => (this.uniforms[_name].value = v),
      });
    });
  }
}

type Props = MeshTransmissionMaterialProps & {
  speed?: number;
  vertexDistort?: number;
};

export const BlobTransmissionMaterial = React.forwardRef(
  (
    {
      buffer,
    ...
    const ref = React.useRef<JSX.IntrinsicElements["meshTransmissionMaterial"]>(
      null!
    );
    ...
    let parent;
    useFrame((state) => {
      ref.current.time = state.clock.getElapsedTime() * (speed || 1);
      // Render only if the buffer matches the built-in and no transmission sampler is set
      if (ref.current.buffer === fboMain.texture && !transmissionSampler) {
        parent = (ref.current as any).__r3f.parent as any;
        if (parent) {
          // Save defaults
